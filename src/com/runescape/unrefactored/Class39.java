...
public final class Class39 {
    ...
    public void method545(JagBuffer jagBuffer, Class29 class29) {
        int i = jagBuffer.readUnsignedByte();
        anIntArray665[0] = i >> 4;
        anIntArray665[1] = i & 0xf;
        if (i != 0) {
            anIntArray668[0] = jagBuffer.readUnsignedShort();
            anIntArray668[1] = jagBuffer.readUnsignedShort();
            int j = jagBuffer.readUnsignedByte();
            for (int k = 0; k < 2; k++) {
                for (int l = 0; l < anIntArray665[k]; l++) {
                    anIntArrayArrayArray666[k][0][l] = jagBuffer.readUnsignedShort();
                    anIntArrayArrayArray667[k][0][l] = jagBuffer.readUnsignedShort();
                }
            ...
            for (int i1 = 0; i1 < 2; i1++) {
                for (int j1 = 0; j1 < anIntArray665[i1]; j1++)
                    if ((j & 1 << i1 * 4 << j1) != 0) {
                        anIntArrayArrayArray666[i1][1][j1] = jagBuffer.readUnsignedShort();
                        anIntArrayArrayArray667[i1][1][j1] = jagBuffer.readUnsignedShort();
                    } else {
