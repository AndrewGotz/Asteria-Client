...
public final class SpotAnim {
    ...
    public static void unpackConfig(StreamLoader streamLoader) {
        JagBuffer jagBuffer = new JagBuffer(streamLoader.getDataForName("spotanim.dat"));
    ...
    private void readValues(JagBuffer jagBuffer) {
        do {
            ...
            if (i == 0)
                return;
            if (i == 1)
                anInt405 = jagBuffer.readUnsignedShort();
            else if (i == 2) {
                anInt406 = jagBuffer.readUnsignedShort();
                if (Animation.anims != null)
                    aAnimation_407 = Animation.anims[anInt406];
            } else if (i == 4)
                anInt410 = jagBuffer.readUnsignedShort();
            else if (i == 5)
                anInt411 = jagBuffer.readUnsignedShort();
            else if (i == 6)
                anInt412 = jagBuffer.readUnsignedShort();
            else if (i == 7)
                anInt413 = jagBuffer.readUnsignedByte();
            else if (i == 8)
                anInt414 = jagBuffer.readUnsignedByte();
            else if (i >= 40 && i < 50)
                anIntArray408[i - 40] = jagBuffer.readUnsignedShort();
            else if (i >= 50 && i < 60)
                anIntArray409[i - 50] = jagBuffer.readUnsignedShort();
            else
                logger.warning("Error unrecognised spotanim config code: " + i);
        } while (true);
    }
