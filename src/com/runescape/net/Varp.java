package com.runescape.net;

import java.util.logging.Logger;

// Decompiled by Jad v1.5.8f. Copyright 2001 Pavel Kouznetsov.
// Jad home page: http://www.kpdus.com/jad.html
// Decompiler options: packimports(3) 

public final class Varp {
    private static Logger logger = Logger.getLogger("RS2Logger");
    public static void unpackConfig(StreamLoader streamLoader) {
        JagBuffer jagBuffer = new JagBuffer(streamLoader.getDataForName("varp.dat"));
        anInt702 = 0;
        int cacheSize = jagBuffer.readUnsignedShort();
        if (cache == null)
            cache = new Varp[cacheSize];
        if (anIntArray703 == null)
            anIntArray703 = new int[cacheSize];
        for (int j = 0; j < cacheSize; j++) {
            if (cache[j] == null)
                cache[j] = new Varp();
            cache[j].readValues(jagBuffer, j);
        }
        if (jagBuffer.currentOffset != jagBuffer.buffer.length)
            logger.warning("varptype load mismatch");
    }

    private void readValues(JagBuffer jagBuffer, int i) {
        do {
            int j = jagBuffer.readUnsignedByte();
            if (j == 0)
                return;
            if (j == 1)
                jagBuffer.readUnsignedByte();
            else if (j == 2)
                jagBuffer.readUnsignedByte();
            else if (j == 3)
                anIntArray703[anInt702++] = i;
            else if (j == 4) {
            } else if (j == 5)
                anInt709 = jagBuffer.readUnsignedShort();
            else if (j == 6) {
            } else if (j == 7)
                jagBuffer.readDWord();
            else if (j == 8)
                aBoolean713 = true;
            else if (j == 10)
                jagBuffer.readString();
            else if (j == 11)
                aBoolean713 = true;
            else if (j == 12)
                jagBuffer.readDWord();
            else if (j == 13) {
            } else
                logger.warning("Error unrecognised config code: " + j);
        } while (true);
    }

    private Varp() {
        aBoolean713 = false;
    }

    public static Varp cache[];
    private static int anInt702;
    private static int[] anIntArray703;
    public int anInt709;
    public boolean aBoolean713;

}
