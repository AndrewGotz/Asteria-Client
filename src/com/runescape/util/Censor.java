...
public final class Censor {
    ...
    public static void loadConfig(StreamLoader streamLoader) {
        JagBuffer jagBuffer = new JagBuffer(streamLoader.getDataForName("fragmentsenc.txt"));
        JagBuffer stream_1 = new JagBuffer(streamLoader.getDataForName("badenc.txt"));
        JagBuffer stream_2 = new JagBuffer(streamLoader.getDataForName("domainenc.txt"));
        JagBuffer stream_3 = new JagBuffer(streamLoader.getDataForName("tldlist.txt"));
        readValues(jagBuffer, stream_1, stream_2, stream_3);
    }

    private static void readValues(JagBuffer jagBuffer, JagBuffer stream_1, JagBuffer stream_2, JagBuffer stream_3) {
        readBadEnc(stream_1);
        readDomainEnc(stream_2);
        readFragmentsEnc(jagBuffer);
        readTldList(stream_3);
    }

    private static void readTldList(JagBuffer jagBuffer) {
        int i = jagBuffer.readDWord();
        aCharArrayArray624 = new char[i][];
        anIntArray625 = new int[i];
        for (int j = 0; j < i; j++) {
            anIntArray625[j] = jagBuffer.readUnsignedByte();
            char ac[] = new char[jagBuffer.readUnsignedByte()];
            for (int k = 0; k < ac.length; k++)
                ac[k] = (char) jagBuffer.readUnsignedByte();
    ...
    private static void readBadEnc(JagBuffer jagBuffer) {
        int j = jagBuffer.readDWord();
        aCharArrayArray621 = new char[j][];
        aByteArrayArrayArray622 = new byte[j][][];
        method493(jagBuffer, aCharArrayArray621, aByteArrayArrayArray622);
    }

    private static void readDomainEnc(JagBuffer jagBuffer) {
        int i = jagBuffer.readDWord();
        aCharArrayArray623 = new char[i][];
        method494(aCharArrayArray623, jagBuffer);
    }

    private static void readFragmentsEnc(JagBuffer jagBuffer) {
        anIntArray620 = new int[jagBuffer.readDWord()];
        for (int i = 0; i < anIntArray620.length; i++)
            anIntArray620[i] = jagBuffer.readUnsignedShort();
    }

    private static void method493(JagBuffer jagBuffer, char ac[][], byte abyte0[][][]) {
        for (int j = 0; j < ac.length; j++) {
            char ac1[] = new char[jagBuffer.readUnsignedByte()];
            for (int k = 0; k < ac1.length; k++)
                ac1[k] = (char) jagBuffer.readUnsignedByte();

            ac[j] = ac1;
            byte abyte1[][] = new byte[jagBuffer.readUnsignedByte()][2];
            for (int l = 0; l < abyte1.length; l++) {
                abyte1[l][0] = (byte) jagBuffer.readUnsignedByte();
                abyte1[l][1] = (byte) jagBuffer.readUnsignedByte();
            }
    ...
    private static void method494(char ac[][], JagBuffer jagBuffer) {
        for (int j = 0; j < ac.length; j++) {
            char ac1[] = new char[jagBuffer.readUnsignedByte()];
            for (int k = 0; k < ac1.length; k++)
                ac1[k] = (char) jagBuffer.readUnsignedByte();
