package com.runescape.world.animable.object;
...
public final class ObjectDef {
    ...
    public static void unpackConfig(StreamLoader streamLoader) {
        jagBuffer = new JagBuffer(streamLoader.getDataForName("loc.dat"));
        JagBuffer jagBuffer = new JagBuffer(streamLoader.getDataForName("loc.idx"));
        int totalObjects = jagBuffer.readUnsignedShort();
        ...
        label0: do {
            ...
            do {
                ...
                if (j == 1) {
                    ...
                    if (k > 0)
                        if (anIntArray773 == null || lowMem) {
                            ...
                            for (int k1 = 0; k1 < k; k1++) {
                                anIntArray773[k1] = jagBuffer.readUnsignedShort();
                                anIntArray776[k1] = jagBuffer.readUnsignedByte();
                            }

                        } else {
                            jagBuffer.currentOffset += k * 3;
                        }
                } else if (j == 2)
                    ...
                    if (l > 0)
                        if (anIntArray773 == null || lowMem) {
                            ...
                            for (int l1 = 0; l1 < l; l1++)
                                anIntArray773[l1] = jagBuffer.readUnsignedShort();

                        } else {
                            jagBuffer.currentOffset += l * 2;
                        }
                } else if (j == 14)
                    anInt744 = jagBuffer.readUnsignedByte();
                else if (j == 15)
                    anInt761 = jagBuffer.readUnsignedByte();
                else if (j == 17)
                    aBoolean767 = false;
                else if (j == 18)
                    aBoolean757 = false;
                else if (j == 19) {
                    i = jagBuffer.readUnsignedByte();
                    if (i == 1)
                        hasActions = true;
                } else if (j == 21)
                    aBoolean762 = true;
                    ...
                    anInt781 = jagBuffer.readUnsignedShort();
                    if (anInt781 == 65535)
                        anInt781 = -1;
                } else if (j == 28)
                    anInt775 = jagBuffer.readUnsignedByte();
                else if (j == 29)
                    aByte737 = jagBuffer.readSignedByte();
                else if (j == 39)
                    aByte742 = jagBuffer.readSignedByte();
                else if (j >= 30 && j < 39) {
                    if (actions == null)
                        actions = new String[5];
                    actions[j - 30] = jagBuffer.readString();
                    ...
                    for (int i2 = 0; i2 < i1; i2++) {
                        modifiedModelColors[i2] = jagBuffer.readUnsignedShort();
                        originalModelColors[i2] = jagBuffer.readUnsignedShort();
                    }

                } else if (j == 60)
                    anInt746 = jagBuffer.readUnsignedShort();
                else if (j == 62)
                    aBoolean751 = true;
                else if (j == 64)
                    aBoolean779 = false;
                else if (j == 65)
                    anInt748 = jagBuffer.readUnsignedShort();
                else if (j == 66)
                    anInt772 = jagBuffer.readUnsignedShort();
                else if (j == 67)
                    anInt740 = jagBuffer.readUnsignedShort();
                else if (j == 68)
                    anInt758 = jagBuffer.readUnsignedShort();
                else if (j == 69)
                    anInt768 = jagBuffer.readUnsignedByte();
                else if (j == 70)
                    anInt738 = jagBuffer.readSignedShort();
                else if (j == 71)
                    anInt745 = jagBuffer.readSignedShort();
                else if (j == 72)
                    anInt783 = jagBuffer.readSignedShort();
                else if (j == 73)
                    aBoolean736 = true;
                else if (j == 74) {
                    aBoolean766 = true;
                    ...
                    if (j != 75)
                        continue;
                    anInt760 = jagBuffer.readUnsignedByte();
                }
                continue label0;
            } while (j != 77);
            anInt774 = jagBuffer.readUnsignedShort();
            if (anInt774 == 65535)
                anInt774 = -1;
            anInt749 = jagBuffer.readUnsignedShort();
            if (anInt749 == 65535)
                anInt749 = -1;
            int j1 = jagBuffer.readUnsignedByte();
            childrenIDs = new int[j1 + 1];
            for (int j2 = 0; j2 <= j1; j2++) {
                childrenIDs[j2] = jagBuffer.readUnsignedShort();
