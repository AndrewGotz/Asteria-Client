...
import com.runescape.applet.drawing.DrawingArea;
import com.runescape.applet.drawing.Sprite;
import com.runescape.net.JagBuffer;
import com.runescape.net.StreamLoader;
import com.runescape.world.MRUNodes;
import com.runescape.world.Texture;
...
public final class ItemDef {
    ...
    public static void unpackConfig(StreamLoader streamLoader) {
        jagBuffer = new JagBuffer(streamLoader.getDataForName("obj.dat"));
        JagBuffer jagBuffer = new JagBuffer(streamLoader.getDataForName("obj.idx"));
        totalItems = jagBuffer.readUnsignedShort();
    ...
    public static int color1 = 10;
    public static int color2 = 7;
    ...
    public static ItemDef forID(int i) {
        ...
        switch (i) {
            case 5514:
                itemDef.name = "Loot bag";
                itemDef.description = "Can be used to store various items.".getBytes();
                break;
        }
        return itemDef;
    }
    ...
    public void colors(int id, int original, int modified) {
        if (id > originalModelColors.length || id > modifiedModelColors.length || id < 0) {
            return;
        }
    ...
    public void readValues(JagBuffer jagBuffer) {
        do {
            int i = jagBuffer.readUnsignedByte();
            if (i == 0)
                return;
            if (i == 1)
                modelID = jagBuffer.readUnsignedShort();
            else if (i == 2)
                name = jagBuffer.readString();
            else if (i == 3)
                description = jagBuffer.readBytes();
            else if (i == 4)
                modelZoom = jagBuffer.readUnsignedShort();
            else if (i == 5)
                modelRotation1 = jagBuffer.readUnsignedShort();
            else if (i == 6)
                modelRotation2 = jagBuffer.readUnsignedShort();
            else if (i == 7) {
                modelOffset1 = jagBuffer.readUnsignedShort();
                if (modelOffset1 > 32767)
                    modelOffset1 -= 0x10000;
            } else if (i == 8) {
                modelOffset2 = jagBuffer.readUnsignedShort();
                ...
                value = jagBuffer.readDWord();
            else if (i == 16)
                membersObject = true;
            else if (i == 23) {
                maleEquip1 = jagBuffer.readUnsignedShort();
                aByte205 = jagBuffer.readSignedByte();
            } else if (i == 24)
                maleEquip2 = jagBuffer.readUnsignedShort();
            else if (i == 25) {
                femaleEquip1 = jagBuffer.readUnsignedShort();
                aByte154 = jagBuffer.readSignedByte();
            } else if (i == 26)
                femaleEquip2 = jagBuffer.readUnsignedShort();
            else if (i >= 30 && i < 35) {
                if (groundActions == null)
                    groundActions = new String[5];
                groundActions[i - 30] = jagBuffer.readString();
                if (groundActions[i - 30].equalsIgnoreCase("hidden"))
                    groundActions[i - 30] = null;
            } else if (i >= 35 && i < 40) {
                if (actions == null)
                    actions = new String[5];
                actions[i - 35] = jagBuffer.readString();
            } else if (i == 40) {
                int j = jagBuffer.readUnsignedByte();
                originalModelColors = new int[j];
                modifiedModelColors = new int[j];
                for (int k = 0; k < j; k++) {
                    originalModelColors[k] = jagBuffer.readUnsignedShort();
                    modifiedModelColors[k] = jagBuffer.readUnsignedShort();
                }
            } else if (i == 78)
                anInt185 = jagBuffer.readUnsignedShort();
            else if (i == 79)
                anInt162 = jagBuffer.readUnsignedShort();
            else if (i == 90)
                anInt175 = jagBuffer.readUnsignedShort();
            else if (i == 91)
                anInt197 = jagBuffer.readUnsignedShort();
            else if (i == 92)
                anInt166 = jagBuffer.readUnsignedShort();
            else if (i == 93)
                anInt173 = jagBuffer.readUnsignedShort();
            else if (i == 95)
                anInt204 = jagBuffer.readUnsignedShort();
            else if (i == 97)
                certID = jagBuffer.readUnsignedShort();
            else if (i == 98)
                certTemplateID = jagBuffer.readUnsignedShort();
            else if (i >= 100 && i < 110) {
                if (stackIDs == null) {
                    stackIDs = new int[10];
                    stackAmounts = new int[10];
                }
                stackIDs[i - 100] = jagBuffer.readUnsignedShort();
                stackAmounts[i - 100] = jagBuffer.readUnsignedShort();
            } else if (i == 110)
                anInt167 = jagBuffer.readUnsignedShort();
            else if (i == 111)
                anInt192 = jagBuffer.readUnsignedShort();
            else if (i == 112)
                anInt191 = jagBuffer.readUnsignedShort();
            else if (i == 113)
                anInt196 = jagBuffer.readSignedByte();
            else if (i == 114)
                anInt184 = jagBuffer.readSignedByte() * 5;
            else if (i == 115)
                team = jagBuffer.readUnsignedByte();
        } while (true);
    }
    ...
    public static JagBuffer jagBuffer;
    public int anInt184;
    public int anInt185;
