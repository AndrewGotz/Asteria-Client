...
import com.runescape.unrefactored.Class6;
...
public final class Sounds {
    ...
    public static void unpack(JagBuffer jagBuffer) {
        aByteArray327 = new byte[0x6baa8];
        aStream_328 = new JagBuffer(aByteArray327);
        Class6.method166();
        do {
            int i = jagBuffer.readUnsignedShort();
            if (i == 65535)
                return;
            aSoundsArray325s[i] = new Sounds();
            aSoundsArray325s[i].method242(jagBuffer);
            anIntArray326[i] = aSoundsArray325s[i].method243();
        } while (true);
    }

    public static JagBuffer method241(int i, int j) {
    ...
    private void method242(JagBuffer jagBuffer) {
        for (int i = 0; i < 10; i++) {
            int j = jagBuffer.readUnsignedByte();
            if (j != 0) {
                jagBuffer.currentOffset--;
                aClass6Array329[i] = new Class6();
                aClass6Array329[i].method169(jagBuffer);
            }
        }

        anInt330 = jagBuffer.readUnsignedShort();
        anInt331 = jagBuffer.readUnsignedShort();
    }
    ...
    private JagBuffer method244(int i) {
        int j = method245(i);
    ...
    private static JagBuffer aStream_328;
