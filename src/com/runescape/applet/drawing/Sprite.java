...
public final class Sprite extends DrawingArea {
    ...
    public Sprite(byte abyte0[], Component component) {
        try {
            ...
            logger.log(Level.WARNING, "Error while building Sprite class!", e);
        }
    }
    ...
    public Sprite(StreamLoader streamLoader, String s, int i) {
        JagBuffer jagBuffer = new JagBuffer(streamLoader.getDataForName(s + ".dat"));
        JagBuffer stream_1 = new JagBuffer(streamLoader.getDataForName("index.dat"));
        stream_1.currentOffset = jagBuffer.readUnsignedShort();
        ...
        for (int l = 0; l < i; l++) {
            ...
            jagBuffer.currentOffset += stream_1.readUnsignedShort() * stream_1.readUnsignedShort();
        ...
        if (i1 == 0) {
            for (int k1 = 0; k1 < j1; k1++)
                myPixels[k1] = ai[jagBuffer.readUnsignedByte()];
        ...
        if (i1 == 1) {
            for (int l1 = 0; l1 < myWidth; l1++) {
                for (int i2 = 0; i2 < myHeight; i2++)
                    myPixels[l1 + i2 * myWidth] = ai[jagBuffer.readUnsignedByte()];
